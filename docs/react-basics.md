...
JavaScript自体(変数・関数・配列の`.map()`など)は知っている前提で、**Reactに特有の考え方**だけを説明する。上から順に読むと、簡単な概念(コンポーネント)から複雑な概念(state・副作用)へと積み上がっていく構成になっている。

先によく出てくる用語だけ簡単に:

| 用語 | 意味 |
|---|---|
| コンポーネント | 画面の部品。中身はただのJavaScript関数 |
| JSX | コンポーネントの中でHTMLのような見た目を書ける構文 |
| props | 親から子のコンポーネントに渡すデータ(関数でいう引数) |
| state | コンポーネントが覚えておく値(`useState`で作る) |
| フック | `useState`や`useEffect`のように`use`で始まる特別な関数。コンポーネントの中でだけ使える |
...
このドキュメントに書かれていないことや、実際のコードで「これは何をしているの?」と疑問に思う箇所があれば、遠慮なく聞いてほしい。実際のコード行を指しながら説明するので、公式ドキュメントを読むより理解しやすいはず。
