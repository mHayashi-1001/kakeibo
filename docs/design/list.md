...
│ 一覧画面                                        │  h1 title
├──────────────────────────────────────────────┤
│ [ 2026年7月      ▼] [内容で検索        ] [並び替え▼]│  フィルタバー
├──────────────────────────────────────────────┤
│ 月次推移(全期間)                                 │
│  収入 ─●───●───●───●─   支出 ─●─╲_╱─●─         │  折れ線グラフ(凡例つき)
├──────────────────────────────────────────────┤
│  収入          支出          差引               │
│  ¥250,000     ¥180,000      ¥70,000            │  サマリー3カラム
├──────────────────────────────────────────────┤
│ 今月の予算消化状況                                │  ※月を1つに絞っている時のみ
│  食費   ████████░░  32,000 / 40,000円            │
│  交通費 ██████████  12,000 / 8,000円 (予算超過)   │
├──────────────────────────────────────────────┤
│ カテゴリ別内訳(支出)    │ カテゴリ別内訳(収入)      │
│  食費   ████████         │  給与   ██████████       │  横棒グラフ2列
│  交通費 ███               │                          │
├──────────────────────────────────────────────┤
│ 日付       │ 内容         │ カテゴリ │ 金額    │      │  テーブルヘッダー
│ 2026-07-01 │ スーパー...  │ 食費     │ -3,200円│[削除]│  行クリックで編集モードに
└──────────────────────────────────────────────┘
```

行クリック時、その行だけが編集モード(日付/内容/種別/カテゴリ/金額の入力欄+保存/キャンセル/削除ボタン)に切り替わる。
...
| 項目 | 内容 |
|---|---|
| 実行SQL | `UPDATE item SET date = $1, name = $2, price = $3, category = $4, type = $5 WHERE id = $6 RETURNING id` |
| 更新対象 | `id`以外の全カラムを一括更新(部分更新ではない) |
...
| 項目 | 内容 |
|---|---|
| 実行SQL | `DELETE FROM item WHERE id = $1 RETURNING id` |
...
| 項目 | 内容 |
|---|---|
| 実行SQL | `SELECT id, date, name, price, category, type FROM item ORDER BY id` |
